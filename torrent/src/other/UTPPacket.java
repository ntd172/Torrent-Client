...
public class UTPPacket {
   ...
   //The selective acknumber. This allows a single packet to
	...
	public UTPPacket(int type, int version, byte[] currentconnectionID, int timestampdifference, int currentwindow, int seq, int ack, byte [] data, InetAddress address, int port,int extension,int selectiveacknumber) {
		this.type = type;
	...
	public DatagramPacket getUDPPacket() {
		...
		byte version_type = (byte)((version & 0xFF) << 4 | (type & 0xFF));
